...
'("03/02/2018", "Updated NDNH new hire DAIL scrubber with INFC case action handling added, and has STAT/JOBS coordinates updated.", "MiKayla Handley and Ilse Ferris, Hennepin County")
'("09/11/2017", "Initial version.", "MiKayla Handley, Hennepin County")
'END CHANGELOG BLOCK =======================================================================================================
...
LOOP UNTIL are_we_passworded_out = false
	...
	'Checks to see if this is post 5/22 or not, and grabs info
	EMReadScreen new_HIRE_SSN, 12, 9, 5
	If left(new_HIRE_SSN, 9) <> "NDNH MEMB" THEN
		new_hire_ssn = left(new_HIRE_SSN, 9)
		bufferLength = 15
	Else
		bufferLength = 18
	END IF
	...
	EMReadScreen new_hire_second_line, 61, row + 1, col - bufferLength
		new_hire_second_line =trim(new_hire_second_line)
	EMReadScreen new_hire_third_line, 61, row + 2, col - bufferLength 'maxis name'
		new_hire_third_line =trim(new_hire_third_line)
	EMReadScreen new_hire_fourth_line, 61, row + 3, col - bufferLength'new hire name'
...
IF match_answer_droplist = "NO - run NEW HIRE" THEN
    ...
    Do
		...
		If bufferlength = 18 THEN 
			EMReadScreen HH_memb, 2, 4, 33
			If right(new_HIRE_SSN, 2) = HH_memb then
    		new_HIRE_SSN = replace(MEMB_SSN, " ", "")
    ...
    CALL write_variable_in_case_note("DATE HIRED: " & date_hired)
		...
		CALL write_variable_in_case_note("* Sent employment verification and DHS-2919 (Verif Request).")
    ...
    'Sending TIKL text.
    CALL write_variable_in_TIKL("Verification of " & employer & "job via NEW HIRE should have returned by now. If not received and processed, take appropriate action." & vbcr & "For all federal matches INFC/HIRE must be cleared.")
