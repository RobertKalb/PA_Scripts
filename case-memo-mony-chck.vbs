...
name_of_script = "case-memo-mony-chck.vbs"
start_time = timer
...
'Dialogs
'>>>>>Main dlg<<<<<
'BeginDialog main_menu, 0, 0, 201, 65, "Cases for COVID-19 from List"
...
'  Text 10, 10, 185, 25, " Modified to only run mode from an Excel file..."
'EndDialog
...
'>>>>>DLG for Excel mode<<<<<
BeginDialog CASE_NOTE_from_excel_dlg, 0, 0, 256, 165, "Case Note Information"
...
'>>>>> This function  will be run from the call to it for CASH I active / MS, GA or RC <<<<<
...
FOR EACH MAXIS_case_number IN case_number_array
	IF MAXIS_case_number <> "" THEN
		' Next check the ACTIVE programs for that case
		...
		EMReadScreen cash_FS_status, 4, 10, 74
	
		IF cash_I_status = "ACTV" and (cash_I_prog = "GA" or cash_I_prog = "RC" or cash_I_prog = "MS") THEN call create_mony_chck_eg ()
		IF cash_II_status = "ACTV" and (cash_II_prog = "GA" or cash_II_prog = "RC" or cash_II_prog = "MS") THEN call create_mony_chck_eg ()
		IF cash_I_status = "ACTV" and (cash_I_prog = "MF" or cash_I_prog = "DW") THEN call create_mony_chck_ea ()
		IF cash_II_status = "ACTV" and (cash_II_prog = "MF" or cash_II_prog = "DW") THEN call create_mony_chck_ea ()
		IF cash_FS_status = "ACTV"  then create_mony_chck_fs ()
