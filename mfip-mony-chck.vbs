...
name_of_script = "mfip-mony-chck.vbs"
start_time = timer
...
For EACH age in age_array
  ...
  IF age <> "" and row > 1 and row < 15 THEN    ' limit the array to the first page for subsequent pop-ups 
    IF age < 22 then
      ...
      CALL write_value_and_transmit("N", row, col + 19)
    END IF
...
FOR EACH MAXIS_case_number IN case_number_array
                IF MAXIS_case_number <> "" THEN
                    ...
                    call navigate_to_MAXIS_screen("stat", "prog")
                                ...
                                'MsgBox (" reading excel for case: " & MAXIS_case_number)
                                'Only running MFIP/DW with case and amount on the EXCEL spread sheet - see other scripts for MFIP and MS
                                IF cash_I_status = "ACTV" and (cash_I_prog = "MF" or cash_I_prog = "DW") THEN call create_mony_chck_mf ()
                                IF cash_II_status = "ACTV" and (cash_II_prog = "MF" or cash_II_prog = "DW") THEN call create_mony_chck_mf ()
					  IF cash_I_status = "REIN" and (cash_I_prog = "MF" or cash_I_prog = "DW") THEN call create_mony_chck_mf ()
                                IF cash_II_status = "REIN" and (cash_II_prog = "MF" or cash_II_prog = "DW") THEN call create_mony_chck_mf ()
