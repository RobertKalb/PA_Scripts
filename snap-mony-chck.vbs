...
name_of_script = "snap-mony-chck.vbs"
start_time = timer
	...
	ELSEIF run_mode = "Excel File" THEN
		...
		FOR i = excel_row TO end_row
			IF objExcel.Cells(i, excel_col).Value <> "" THEN
				...
				mfip_add_array = snap_add_array & objExcel.Cells(i, excel_col+2).Value  & "$$" 
				set_add_array  = snap_add_array & objExcel.Cells(i, excel_col+3).Value  & "$$" 
			END IF
		NEXT
	END IF
...
set_add_array = trim(set_add_array)
set_add_array = split(set_add_array, "$$")
...
FOR EACH MAXIS_case_number IN case_number_array
	IF MAXIS_case_number <> "" THEN
		...
		'Only running SNAP with case and amount on the EXCEL spread sheet - see other scripts for MFIP and MS
