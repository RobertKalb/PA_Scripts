...
'' THIS FUNCTIONALITY NEEDS TO BE COMMENTED OUT OF THE MAIN MENU SCRIPT - THE SCRIPT CLASS IS ALREADY DECLARED IN THE FUNCTIONS LIBRARY.
...
For i = 0 to ubound(script_array_BULK_main)
	If ButtonPressed = script_array_BULK_main(i).button then call launch_selected_script(script_repository & "bulk\" & script_array_BULK_main(i).file_name)
Next

For i = 0 to ubound(script_array_BULK_enhanced_list)
	If ButtonPressed = script_array_BULK_enhanced_list(i).button then call launch_selected_script(script_repository & "bulk\" & script_array_BULK_enhanced_list(i).file_name)
Next

For i = 0 to ubound(script_array_BULK_stat_list)
	If ButtonPressed = script_array_BULK_stat_list(i).button then call launch_selected_script(script_repository & "bulk\" & script_array_BULK_stat_list(i).file_name)
Next
stopscript
